...
export default {
  ...
  props: {
    ...
    particleShape: {
      ...
      validator: function (value) {
        return ['capsule', 'sphere', 'box', 'tetrahedron'].indexOf(value) !== -1
      }
    },
    fieldStrength: {
      type: Number,
  ...
  methods: {
    ...
    setupScene: function () {
      ...
      // Create camera
      this.camera = new window.THREE.PerspectiveCamera(35, clientWidth / clientHeight, 0.1, 1000);
      this.camera.position.z = 50;

      // Get viewport dimensions at camera depth
      const viewport = this.getViewportAtDepth(this.camera, this.camera.position.z);
      ...
      const material = new window.THREE.MeshBasicMaterial({ color: this.color });
      this.mesh = new window.THREE.InstancedMesh(geometry, material, this.count);
    ...
    animate: function () {
      ...
      this.particles.forEach((particle, i) => {
        ...
        const finalScale =
          scaleFactor * (0.8 + Math.sin(t * this.pulseSpeed) * 0.2 * this.particleVariance) * this.particleSize;
