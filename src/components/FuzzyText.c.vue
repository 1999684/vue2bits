...
export default {
  ...
  methods: {
    waitForFont: function(fontFamily, fontWeight, fontSize) {
      ...
      return new Promise(function(resolve) {
        if (document.fonts && document.fonts.check) {
          ...
          document.fonts.load(fontString).then(function() {
    ...
    initCanvasInternal: function() {
      ...
      var computedFontFamily = this.fontFamily === 'inherit' ? 
        (window.getComputedStyle(canvas).fontFamily || 'sans-serif') : 
        this.fontFamily;

      var fontSizeStr = typeof this.fontSize === 'number' ? this.fontSize + 'px' : this.fontSize;
      var numericFontSize;
      ...
      var self = this;
      this.waitForFont(computedFontFamily, this.fontWeight, fontSizeStr).then(function(fontLoaded) {
        if (!fontLoaded) {
          console.warn('Font not loaded: ' + computedFontFamily);
        }

        self.setupCanvas(canvas, ctx, computedFontFamily, fontSizeStr, numericFontSize);
      });
    },
    setupCanvas: function(canvas, ctx, computedFontFamily, fontSizeStr, numericFontSize) {
      if (this.isCancelled) return;
      ...
      var fontString = this.fontWeight + ' ' + fontSizeStr + ' ' + computedFontFamily;
