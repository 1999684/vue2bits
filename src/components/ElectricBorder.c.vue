...
export default {
  ...
  data: function() {
    return {
      rawId: 'id-' + (Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15)),
      filterId: null,
      ro: null
    };
  },
  ...
  computed: {
    ...
    bgGlowStyle: function() {
      return {
        ...
        opacity: 0.3,
        zIndex: -1,
        background: 'linear-gradient(-30deg, ' + hexToRgba(this.color, 0.8) + ', transparent, ' + this.color + ')'
      };
    }
  },
  mounted: function() {
  ...
  methods: {
    ...
    updateAnim: function() {
      ...
      var width = Math.max(1, Math.round(host.clientWidth || host.getBoundingClientRect().width || 0));
      var height = Math.max(1, Math.round(host.clientHeight || host.getBoundingClientRect().height || 0));

      var dyAnims = Array.prototype.slice.call(svg.querySelectorAll('feOffset > animate[attributeName="dy"]'));
      if (dyAnims.length >= 2) {
        dyAnims[0].setAttribute('values', height + '; 0');
        dyAnims[1].setAttribute('values', '0; -' + height);
      }

      var dxAnims = Array.prototype.slice.call(svg.querySelectorAll('feOffset > animate[attributeName="dx"]'));
      if (dxAnims.length >= 2) {
...
<template>
  <div ref="rootRef" :class="['electric-border', className]" :style="customStyle">
    <svg
      ref="svgRef"
    ...
    >
      <defs>
        <filter :id="filterId" color-interpolation-filters="sRGB" x="-200%" y="-200%" width="500%" height="500%">
          ...
          <feComposite in="offsetNoise1" in2="offsetNoise2" operator="arithmetic" k2="1" k3="1" result="verticalNoise" />
          <feComposite in="offsetNoise3" in2="offsetNoise4" operator="arithmetic" k2="1" k3="1" result="horizontalNoise" />
    ...
    <div class="electric-border-glow" :style="inheritRadius">
      <div ref="strokeRef" class="electric-border-glow-div" :style="strokeStyle" />
