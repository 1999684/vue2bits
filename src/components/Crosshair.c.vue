...
export default {
  ...
  methods: {
    ...
    initCrosshair: function () {
      ...
      const handleMouseMove = function (ev) {
        ...
        if (self.containerRef) {
          ...
          if (
            ...
            if (window.gsap) {
              window.gsap.to(
                [self.$refs.lineHorizontalRef, self.$refs.lineVerticalRef],
                { opacity: 0 }
              );
            } else {
              // 备用方案：如果 GSAP 不可用，直接设置样式
              if (self.$refs.lineHorizontalRef)
                self.$refs.lineHorizontalRef.style.opacity = 0;
              if (self.$refs.lineVerticalRef)
                self.$refs.lineVerticalRef.style.opacity = 0;
            }
          } else {
            if (window.gsap) {
              window.gsap.to(
              ...
              // 备用方案：如果 GSAP 不可用，直接设置样式
              if (self.$refs.lineHorizontalRef)
                self.$refs.lineHorizontalRef.style.opacity = 1;
              if (self.$refs.lineVerticalRef)
                self.$refs.lineVerticalRef.style.opacity = 1;
            }
          }
        }
      };

      (this.containerRef || window).addEventListener(
        "mousemove",
        handleMouseMove
      );
      ...
      if (window.gsap) {
        window.gsap.set(
          [this.$refs.lineHorizontalRef, this.$refs.lineVerticalRef],
          { opacity: 0 }
        );
      } else {
        // 备用方案
        if (this.$refs.lineHorizontalRef)
          this.$refs.lineHorizontalRef.style.opacity = 0;
        if (this.$refs.lineVerticalRef)
          this.$refs.lineVerticalRef.style.opacity = 0;
      }
          ...
          .timeline({
            ...
            onStart: function () {
              if (self.$refs.lineHorizontalRef && self.$refs.lineVerticalRef) {
                self.$refs.lineHorizontalRef.style.filter =
                  "url(#filter-noise-x)";
                self.$refs.lineVerticalRef.style.filter =
                  "url(#filter-noise-y)";
              }
            },
            onUpdate: function () {
              // 更新 SVG 滤镜值
              const filterX = document.querySelector(
                "#filter-noise-x feTurbulence"
              );
              const filterY = document.querySelector(
                "#filter-noise-y feTurbulence"
              );
              if (filterX)
                filterX.setAttribute(
                  "baseFrequency",
                  self.primitiveValues.turbulence.toString()
                );
              if (filterY)
      ...
      this.render = function () {
        ...
        if (window.gsap) {
          if (self.$refs.lineVerticalRef)
            window.gsap.set(self.$refs.lineVerticalRef, {
              x: self.renderedStyles.tx.previous,
            });
          if (self.$refs.lineHorizontalRef)
            window.gsap.set(self.$refs.lineHorizontalRef, {
              y: self.renderedStyles.ty.previous,
            });
        } else {
          // 备用方案：直接设置样式
          if (self.$refs.lineVerticalRef)
            self.$refs.lineVerticalRef.style.transform = `translateX(${self.renderedStyles.tx.previous}px)`;
          if (self.$refs.lineHorizontalRef)
            self.$refs.lineHorizontalRef.style.transform = `translateY(${self.renderedStyles.ty.previous}px)`;
        }
