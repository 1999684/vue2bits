...
export default {
  ...
  mounted: function() {
    ...
    if (typeof window !== 'undefined' && typeof window.THREE !== 'undefined') {
      ...
      console.warn('THREE.js is not available. Please include it before using AsciiText component.');
    }
  },
  ...
  methods: {
    initAscii: function() {
      if (typeof window === 'undefined' || !this.$refs.containerRef || !window.THREE) return;
      ...
      // 创建相机
      var camera = new window.THREE.PerspectiveCamera(45, width / height, 1, 1000);
      camera.position.z = 30;
      
      // 创建渲染器
      var renderer = new window.THREE.WebGLRenderer({ antialias: false, alpha: true });
